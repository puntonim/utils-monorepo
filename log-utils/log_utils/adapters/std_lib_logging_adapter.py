...
class StdLibLoggingAdapter(BaseLogAdapter):
    ...
    def debug(self, message: str, extra: dict | None, **kwargs):
        ...
        logging.debug(message, stacklevel=4, **kwargs)

    def info(self, message: str, extra: dict | None, **kwargs):
        message = self._format_message(message, extra)
        logging.info(message, stacklevel=4, **kwargs)

    def warning(self, message: str, extra: dict | None, **kwargs):
        message = self._format_message(message, extra)
        logging.warning(message, stacklevel=4, **kwargs)

    def error(self, message: str, extra: dict | None, **kwargs):
        message = self._format_message(message, extra)
        logging.error(message, stacklevel=4, **kwargs)

    def critical(self, message: str, extra: dict | None, **kwargs):
        message = self._format_message(message, extra)
        logging.critical(message, stacklevel=4, **kwargs)

    def exception(self, message: str | None, extra: dict | None, **kwargs):
        message = self._format_message(message, extra)
        logging.error(
